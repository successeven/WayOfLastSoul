...
public class HeroMotor : CharacterMotor
{
    ...
    private IEnumerator DoAttack(string AudioClipName, float time)
    {
        ...
        AttackItem item = Hero.instance.Manager._attackItems.Where(x => x._ID == (int)currentAttackEnum).FirstOrDefault();
        if (item != null)
        {
            GameObject particalObj;
            if (Hero.instance.transform.localScale.x > 0)
                particalObj = Instantiate(item._EffectRightSide, transform.position, Quaternion.identity);
            else
                particalObj = Instantiate(item._EffectLeftSide, transform.position, Quaternion.identity);

            Destroy(particalObj, 2f);
        }
